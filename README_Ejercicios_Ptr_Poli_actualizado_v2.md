...
> **Importante sobre los fragmentos de código de esta guía**  
> Los fragmentos incluidos en las secciones de implementación son **pistas orientativas**, no una solución completa lista para copiar. Su propósito es ayudarte a reconocer la sintaxis, el flujo lógico y las instrucciones clave que debes usar. Cada estudiante debe completar su propia implementación, agregar validaciones, organizar los archivos correctamente y probar que su código funciona.
...
## A2.5 Criterios de implementación y pista orientativa

Tu implementación debe recorrer todos los elementos usando un índice lineal `k`. La siguiente idea muestra **solo el núcleo del recorrido**, sin incluir la firma completa del método, las validaciones ni el manejo de errores:

```cpp
// Pista orientativa: recorrido elemento por elemento.
// No es la solución completa del ejercicio.
for (int k = 0; k < total_de_elementos; ++k) {
    // guardar en result la suma del elemento k de la matriz actual
    // y el elemento k de la matriz recibida como parámetro
}
```

El alumno debe decidir cómo conectar esta idea con los atributos reales de la clase, las validaciones obligatorias y la firma del método solicitada.
...
> **Nota:** Este uso en `main.cpp` es una prueba mínima de comportamiento. No sustituye la implementación completa ni las pruebas de cada ejercicio.
...
## B1.3 Criterios de implementación y pista orientativa en `src/MatrixOp.cpp`
...
para cada posición k:
    resultado[k] = matriz_actual[k] operación other[k]
devolver resultado
```

> **Pista:** en `operator+`, la operación es suma; en `operator-`, la operación es resta. No copies este bloque como código: úsalo como mapa mental para construir tu implementación en C++.
...
## B2.4 Criterios de implementación y pista orientativa
...
La lógica esperada puede leerse así:

```text
si el arreglo es nulo:
    reportar error

si el tamaño es inválido:
    reportar error

definir el primer elemento como máximo inicial
...
> **Pista:** el reto no es memorizar el algoritmo, sino traducir esta lógica a una función plantilla `template<typename T>` que funcione con distintos tipos de datos.
